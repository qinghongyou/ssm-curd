...
<body>
	...
	<div class="container">
		...
		<div class="row">
			<div class="col-md-12">
				<table class="table table-striped" style="margin-top:5px;" id="empsTable">
					<thead>
						<tr>
							<th>
	...
	<script type="text/javascript">
		...
		function validate_add_form(){
			...
			if(!regName.test(empName)){
				//alert("用户名可以是2-5位的中文或者6-16的英文和数字组合");
				show_validate_msg("#empName_add_input", "error", "用户名可以是2-5位的中文或者6-16的英文和数字组合");
				return false;
			}else{
			...
			$.ajax({
				...
				success:function(result){
					if(result.code == 100){
						...
						// console.log(result)
						// 有哪个字段的错误信息就显示哪个字段的
						if(undefined != result.extend.errorsField.email){
							show_validate_msg("#email_add_input", "error", result.extend.errorsField.email);
						}
						if(undefined != result.extend.errorsField.empName){
							show_validate_msg("#empName_add_input", "error", result.extend.errorsField.empName);
						}
					}
		...
		$("#emp_update_btn").click(function(){
			...
			// 2.发送ajax 请求保存更新员工信息
			$.ajax({
				url:"${LOCAL_PATH}/emp/"+$(this).attr("edit-id"),
				type:"PUT",
		...
		$(document).on("click", ".delete_btn", function(){
			...
			if(confirm("确认删除【"+empName+"】吗？")){
				// 确认，发送ajax 请求删除即可
				$.ajax({
					url:"${LOCAL_PATH}/emp/"+$(this).attr("del-id"),
					type:"DELETE",
					success:function(result){
						alert(result.msg);
						// 2.来到本页面
